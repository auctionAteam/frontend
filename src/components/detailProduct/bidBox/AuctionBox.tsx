...
const AuctionBoxStyle = styled.div`
  .bitTextBox {
    display: flex;
    align-items: center;
    margin-bottom: 8px;
...
const AuctionBox: React.FC<AuctionBoxProps> = ({
  nowBidMoney,
  priceUnit,
  bidNum,
  handleBid,
  ...
  return (
    <AuctionBoxStyle>
      ...
      <div className="bitTextBox">
        ...
        <div className="bidMoney">{Math.round(nowBidMoney + priceUnit * 1)}원</div>
      </div>
      <div className="buttonBox">
        {rates.map((rate) => (
          <button className="moneyBtn" key={rate} onClick={() => handleBid(rate)}>
            {Math.round(nowBidMoney + priceUnit * rate)}원
          </button>
        ))}
      </div>
